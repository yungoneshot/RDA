...
public class register extends JFrame {
        ...
        btnRegister.addActionListener(e -> {
            ...
            if (userId > 0) {
                if (rbZdravstveniRadnik.isSelected()) {
                    ...
                    dispose();
                    UnosPacijent unosPacijentWindow = new UnosPacijent(userId, username);
    ...
    private boolean saveZdravstveniRadnik(int userId, String ime, String prezime) {
        ...
        try (Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD)) {
            // Include id_user in the insert
            String sql = "INSERT INTO ZDRAVSTVENI_RADNIK (ID_Zdravstveni_radnik, Ime_zdravstveni_radnik, Prezime_zdravstveni_radnik, id_user) VALUES (?, ?, ?, ?)";
            PreparedStatement ps = conn.prepareStatement(sql);
            ps.setInt(1, userId); // Set as both ID_Zdravstveni_radnik and id_user
            ps.setString(2, ime);
            ps.setString(3, prezime);
            ps.setInt(4, userId); // Also assign to id_user
